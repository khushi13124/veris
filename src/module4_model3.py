"""
src/module4_model3.py
=====================
Module 4 — Model 3: ResNet-34 Severity Classifier — Neural Foraminal Narrowing (NFN)
...
Conditions trained together:
    left_neural_foraminal_narrowing   (5 levels × left)
    right_neural_foraminal_narrowing  (5 levels × right)
...
-----------
  model3_best.pt     — best val weighted log-loss
  model3_latest.pt   — overwritten every epoch  (resume target)
  model3_epoch{N}.pt — every 10 epochs

Run
---
  python -m src.module4_model3
  python -m src.module4_model3 --smoke-only
  python -m src.module4_model3 --run-id my_run_001
"""
...
def build_nfn_datasets(
    ...
    """
    Combine left and right NFN into single train and val datasets.
    Left and right are separate annotation rows → separate samples.
    Class weights computed from the combined train dataset.
    """
    train_parts = []
    val_parts   = []
...
def smoke_test(
    ...
    try:
        ...
        if losses[-1] >= losses[0]:
            issues.append(f"Loss did not decrease: {losses[0]:.6f} → {losses[-1]:.6f}")
        else:
...
def _plot_curves(
    ...
    fig.suptitle(f"Model 3 (NFN) training curves — {run_id}", fontsize=11)
...
def run(run_id: str = None, smoke_only: bool = False):
    run_id     = run_id or make_run_id("m4m3")
    ...
    logger.info("=" * 65)
    logger.info(f"Module 4 — Model 3 (NFN Classifier) | run_id={run_id}")
    ...
    try:
        ...
        logger.info("── Building NFN datasets (left + right combined) ────────")
        train_ds, val_ds = build_nfn_datasets(train_df, val_df, logger)

        logger.info(f"\n  Total train samples (L+R NFN): {len(train_ds):,}")
        logger.info(f"  Total val   samples (L+R NFN): {len(val_ds):,}")

        if len(train_ds) == 0:
            raise RuntimeError("No train patches found for NFN conditions.")
    ...
    write_run_summary(
        ...
        notes=f"NFN classifier (L+R). Val weighted log-loss={best_val:.4f}",
    )
    log_event(run_id, SCRIPT_NAME, "script_complete", end_time=end_time)
    logger.info(f"\nModule 4 Model 3 complete.")


def main():
    parser = argparse.ArgumentParser(
        description="Module 4 — Model 3: NFN Classifier"
    )
