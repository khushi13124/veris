"""
src/module4_model3_left.py
===========================
Module 4 — Model 3 LEFT: ResNet-34 Severity Classifier — NFN Left side only
...
Series : Sagittal T1
Condition: left_neural_foraminal_narrowing only (5 levels × left)
...
-----------
  model3_left_best.pt     — best val weighted log-loss
  model3_left_latest.pt   — overwritten every epoch  (resume target)
  model3_left_epoch{N}.pt — every 10 epochs

Run
---
  python -m src.module4_model3_left
  python -m src.module4_model3_left --smoke-only
  python -m src.module4_model3_left --run-id my_run_001
"""
...
SCRIPT_NAME = "module4_model3_left"

# Both NFN conditions trained in the same model
CONDITIONS  = [
    "left_neural_foraminal_narrowing",   # left side only
]
N_CLASSES   = 3
...
def _plot_curves(
    ...
    fig.suptitle(f"Model 3 LEFT (NFN Left) training curves — {run_id}", fontsize=11)
...
def run(run_id: Optional[str] = None, smoke_only: bool = False):
    run_id     = run_id or make_run_id("m4m3L")
    ...
    logger.info("=" * 65)
    logger.info(f"Module 4 — Model 3 LEFT (NFN Left Classifier) | run_id={run_id}")
    ...
    try:
        ...
        logger.info("── Building NFN dataset (left side only) ──────────────")
        train_ds, val_ds = build_nfn_datasets(train_df, val_df, logger)

        logger.info(f"\n  Total train samples (left NFN): {len(train_ds):,}")
        logger.info(f"  Total val   samples (left NFN): {len(val_ds):,}")

        if len(train_ds) == 0:
            raise RuntimeError("No train patches found for left NFN condition.")
    ...
    write_run_summary(
        ...
        notes=f"NFN left classifier. Val weighted log-loss={best_val:.4f}",
    )
    log_event(run_id, SCRIPT_NAME, "script_complete", end_time=end_time)
    logger.info(f"\nModule 4 Model 3 LEFT complete.")


def main():
    parser = argparse.ArgumentParser(
        description="Module 4 — Model 3 LEFT: NFN Left-side Classifier"
    )
