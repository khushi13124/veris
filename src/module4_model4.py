"""
src/module4_model4.py
=====================
Module 4 — Model 4: ResNet-34 Severity Classifier — Subarticular Stenosis (SS)

Input  : 3-slice stacked Axial T2 patches (3, 128, 128) from .npy files
...
Series : Axial T2
Conditions trained together:
    left_subarticular_stenosis   (5 levels × left)
    right_subarticular_stenosis  (5 levels × right)
    → 10 annotations per study, all treated as separate samples
...
-----------
  model4_best.pt     — best val weighted log-loss
  model4_latest.pt   — overwritten every epoch  (resume target)
  model4_epoch{N}.pt — every 10 epochs

Run
---
  python -m src.module4_model4
  python -m src.module4_model4 --smoke-only
  python -m src.module4_model4 --run-id my_run_001
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset builder — combines both SS conditions
# ─────────────────────────────────────────────────────────────────────────────

def build_ss_datasets(
    train_df: pd.DataFrame,
    val_df:   pd.DataFrame,
    logger,
) -> Tuple[RSNADataset, RSNADataset]:
    """
    Combine left and right subarticular stenosis into single datasets.
    Left and right have separate annotation coordinates → separate samples.
    """
    train_parts = []
    val_parts   = []
...
def smoke_test(
    ...
    try:
        ...
        if head_grad_norm == 0.0:
            issues.append("Head gradient norm is zero")
    ...
    try:
        ...
        # Use first-half vs second-half mean — robust to per-batch noise
...
def _plot_curves(
    ...
    fig.suptitle(f"Model 4 (SS) training curves — {run_id}", fontsize=11)
...
def run(run_id: str = None, smoke_only: bool = False):
    run_id     = run_id or make_run_id("m4m4")
    ...
    logger.info("=" * 65)
    logger.info(f"Module 4 — Model 4 (SS Classifier) | run_id={run_id}")
    ...
    try:
        ...
        logger.info("── Building SS datasets (left + right combined) ─────────")
        train_ds, val_ds = build_ss_datasets(train_df, val_df, logger)

        logger.info(f"\n  Total train samples (L+R SS): {len(train_ds):,}")
        logger.info(f"  Total val   samples (L+R SS): {len(val_ds):,}")

        if len(train_ds) == 0:
            raise RuntimeError("No train patches found for SS conditions.")
    ...
    write_run_summary(
        ...
        notes=f"SS classifier (L+R). Val weighted log-loss={best_val:.4f}",
    )
    log_event(run_id, SCRIPT_NAME, "script_complete", end_time=end_time)
    logger.info(f"\nModule 4 Model 4 complete.")


def main():
    parser = argparse.ArgumentParser(
        description="Module 4 — Model 4: SS Classifier"
    )
